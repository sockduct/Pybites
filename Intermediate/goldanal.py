...
'''
In this Bite you will analyze how the price of gold evolved over the years
1950-2018. We loaded the gold prices (per ounce per year) into the template for
you (source:
https://pkgstore.datahub.io/core/gold-prices/annual_csv/data/343f626dd4f7bae813cfaac23fccd1bc/annual_csv.csv
).

Parse the gold_prices multiline string into a useful data structure and
calculate which years the price decreased and increased the most. Return those 2
years as a tuple:
...
def years_gold_value_decreased(gold_prices: str = gold_prices) -> tuple[int, int]:
    ...
    """
    gold_data = {}
    for gpset in gold_prices.split():
        year_month, price = gpset.split(',')
        gold_data[year_month.split('-')[0]] = float(price)
